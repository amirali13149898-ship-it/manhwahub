...
export function signInWithGoogle(appName = 'مانهوا هاب') {
  const url = buildGoogleUrl(appName);
  if (!url) throw new Error('ورود گوگل در این محیط پیکربندی نشده است.');
  window.open(url, 'google-auth', isMobile() ? '' : 'width=500,height=650');
...
export async function handleGoogleRedirect() {
  ...
  if (!error) try { window.close(); } catch { /* noop */ }
}
